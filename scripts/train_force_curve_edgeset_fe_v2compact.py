...
class EdgeSetCurveMLP(nn.Module):
    def __init__(self, edge_in_dim, seq_len=100, hidden=192, edge_hidden=192, dropout=0.0):
        super().__init__()
...
def member_neighbor_indices(members):
    """Return per-member neighbor index arrays for edges sharing either endpoint."""
    members = np.asarray(members, dtype=np.int64)
    incident = {}
    for e, (i, j) in enumerate(members):
        incident.setdefault(int(i), []).append(e)
        incident.setdefault(int(j), []).append(e)
    neigh = []
    degree = np.zeros(len(members), dtype=np.float32)
    for e, (i, j) in enumerate(members):
        ns = set(incident[int(i)]) | set(incident[int(j)])
        ns.discard(e)
        arr = np.asarray(sorted(ns), dtype=np.int64)
        neigh.append(arr)
        degree[e] = float(len(arr))
    return neigh, degree


def build_neighbor_features(members, area_mult, elastic_demand, failure_margin, demand_ratio):
    """Pre-run one-hop member-neighborhood features.

    All inputs are per kept run/design and per edge with shape (n_runs, n_edges).
    A neighboring member shares either endpoint node. The member itself is excluded.
    """
    area_mult = np.asarray(area_mult, dtype=np.float32)
    elastic_demand = np.asarray(elastic_demand, dtype=np.float32)
    failure_margin = np.asarray(failure_margin, dtype=np.float32)
    demand_ratio = np.asarray(demand_ratio, dtype=np.float32)
    n_runs, n_edges = area_mult.shape
    neigh, degree = member_neighbor_indices(members)
    out = np.zeros((n_runs, n_edges, 9), dtype=np.float32)
    for e, idx in enumerate(neigh):
        out[:, e, 8] = degree[e]
        if idx.size == 0:
            # Degenerate isolated edge fallback: use self values, zero std/degree.
            out[:, e, 0] = area_mult[:, e]
            out[:, e, 1] = 0.0
            out[:, e, 2] = elastic_demand[:, e]
            out[:, e, 3] = elastic_demand[:, e]
            out[:, e, 4] = failure_margin[:, e]
            out[:, e, 5] = failure_margin[:, e]
            out[:, e, 6] = demand_ratio[:, e]
            out[:, e, 7] = demand_ratio[:, e]
            continue
        a = area_mult[:, idx]
        d = elastic_demand[:, idx]
        m = failure_margin[:, idx]
        r = demand_ratio[:, idx]
        out[:, e, 0] = a.mean(axis=1)
        out[:, e, 1] = a.std(axis=1)
        out[:, e, 2] = d.mean(axis=1)
        out[:, e, 3] = d.max(axis=1)
        out[:, e, 4] = m.mean(axis=1)
        out[:, e, 5] = m.min(axis=1)
        out[:, e, 6] = r.mean(axis=1)
        out[:, e, 7] = r.max(axis=1)
    return out


def within_row_percentile(values):
    """Ordinal within-row percentile in [0, 1], computed independently per run.

    This intentionally does not use any dataset-wide thresholds or split statistics.
    """
    values = np.asarray(values, dtype=np.float32)
    n_runs, n_edges = values.shape
    if n_edges <= 1:
        return np.zeros_like(values, dtype=np.float32)
    order = np.argsort(values, axis=1, kind="mergesort")
    ranks = np.empty_like(order, dtype=np.float32)
    row = np.arange(n_runs)[:, None]
    ranks[row, order] = np.arange(n_edges, dtype=np.float32)[None, :]
    return (ranks / float(n_edges - 1)).astype(np.float32)


def build_global_rank_features(area_mult, elastic_demand, failure_margin, demand_ratio):
    """Per-run global member percentiles; no dataset-wide statistics are used."""
    feats = np.stack([
        within_row_percentile(area_mult),
        within_row_percentile(elastic_demand),
        within_row_percentile(failure_margin),
        within_row_percentile(demand_ratio),
    ], axis=-1).astype(np.float32)
    return feats


def build_features(designs, targets, failure_pack, include_elastic=True, feature_set="base"):
    valid_feature_sets = {"base", "neighbor_only", "rank_only", "v2_compact"}
    if feature_set not in valid_feature_sets:
        raise ValueError(f"feature_set must be one of {sorted(valid_feature_sets)}, got {feature_set!r}")
    ...
    if include_elastic:
        ...
        elastic_demand = np.asarray(abs_strain, dtype=np.float32)
        demand_ratio = elastic_demand / np.clip(run_fs, 1e-12, None)
        margin = run_fs / np.clip(elastic_demand, 1e-12, None)
        ...
        elastic_demand = np.zeros((n_runs, E), dtype=np.float32)
        demand_ratio = np.zeros((n_runs, E), dtype=np.float32)
        margin = np.zeros((n_runs, E), dtype=np.float32)
        margin_feats = np.zeros((n_runs, E, 0), dtype=np.float32)

    geom_b = np.broadcast_to(geom_edge[None, :, :], (n_runs, E, geom_edge.shape[1])).copy()
    base_parts = [geom_b, area_feats, fs_log, run_elastic, margin_feats]
    extra_parts = []
    added_feature_names = []
    ...
    if feature_set in {"neighbor_only", "v2_compact"}:
        extra_parts.append(build_neighbor_features(members, run_area, elastic_demand, margin, demand_ratio))
        added_feature_names.extend(neighbor_feature_names)
    if feature_set in {"rank_only", "v2_compact"}:
        extra_parts.append(build_global_rank_features(run_area, elastic_demand, margin, demand_ratio))
        added_feature_names.extend(rank_feature_names)

    edge_features = np.concatenate(base_parts + extra_parts, axis=-1).astype(np.float32)
    if not np.isfinite(edge_features).all():
        bad = int(edge_features.size - np.isfinite(edge_features).sum())
        raise ValueError(f"Non-finite edge features generated: {bad} NaN/Inf values")
    ...
    return {
        ...
        "feature_info": {
            ...
            "base_edge_feature_dim": int(len(base_feature_names)),
            "added_edge_feature_names": added_feature_names,
            "added_edge_feature_dim": int(len(added_feature_names)),
...
def main():
    ...
    ap.add_argument("--feature-set", choices=["base", "neighbor_only", "rank_only", "v2_compact"], default="base")
    ...
    data = build_features(designs, targets, failure_pack, include_elastic=not args.no_elastic, feature_set=args.feature_set)
